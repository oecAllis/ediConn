...
public class EdiStructure {
  ...
  public static EdiStructure fromStream(InputStream input) throws JAXBException {
    ...
    Map<String, Loop> loopMap = ediStructure.loops.stream()
        .collect(Collectors.toMap(Loop::getId, Function.identity(),
            (x, y) -> {
              throw new RuntimeException(
                  "Multiple top-level loops with the same id: '" + x.getId() + "'");
            }
        ));
  ...
  public Loop getRootLoop() {
    return loops.size() == 1 ? loops.get(0)
        : loops.stream()
            ...
            .findAny()
            .orElseThrow(() -> new RuntimeException("One top-level loop should have id='ROOT'"));
  }
